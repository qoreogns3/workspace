...
public class BoardController {
  ...
  public BoardController(){
    ...
    //리스트에 게시글을 5개 저장
    boardList.add(new BoardDTO(1, "첫번째 글", "김자바", 5));
    boardList.add(new BoardDTO(2, "두번째 글", "이자바", 0));
    boardList.add(new BoardDTO(3, "세번째 글", "김자바", 10));
    boardList.add(new BoardDTO(4, "네번째 글", "이자바", 3));
    boardList.add(new BoardDTO(5, "다섯번째 글", "김자바", 7));
  }
  ...
  //문법 : @Pathvariable("url 파라메터 이름") 해당 테이터를 받을 변수)
  ...
  //age라는 이름의 url parameter를 정수 자료형 num에 저장
  //name라는 이름의 url parameter를 문자열 자료형 name에 저장
  @GetMapping("/{age}/{name}")
  ...
  //@RequestBody 어노테이션으로 전받발음
  //@RequestBody 어노테이션 다음에 클래스 자료형을 매개변수로 선언하면 해당 클래스에 데이터를 받을수 있음
  //단, 해당 클래스의 멤버변수명은 요청 시 전달되는 객체의 key값과 동일해야 함!
  @PostMapping("")
  ...
  //게시글 수정 API
  //URL -> (PUT) localhost:8080/boards/1
  //게시글의 제목("집에 갑시다")과 작성자("관리자") 변경
  @PutMapping("/{boardNum}")
