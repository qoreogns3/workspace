...
SELECT 70.5, CEIL(70.4), FLOOR(70.7), ROUND(70.5);
...
#예제> EMP 테이블에서 사번이 짝수인 사원의 모든 정보를 조회
...
#제목에 hello 글자기 포함된 게시글
-- SELECT *
-- FROM BOARD
-- WHERE LOWER(TITLE) LIKE'%hello%';
...
SELECT ENAME
	...
	, CASE DEPTNO
		...
		ELSE '인사부' 
	END
	AS '부서명' #컬럼 명을 부서명으로 바꿈, AS 생략가능
	, CASE
		WHEN DEPTNO = 10 THEN '개발부'
		WHEN DEPTNO = 20 THEN '영엉부'
		ELSE '인사부' 
	END '부서명'
FROM EMP;
...
SELECT TITLE, AUTHOR FROM BOOK_STORE WHERE MOD(BOOK_ID, 2) = 1; 
...
SELECT CONCAT(IFNULL(DISCOUNT_RATE, 0) * 100, '%') '할인율' FROM BOOK_STORE;  

-- IF 함수를 사용하여 재고(STOCK)가 0이면 '품절', 아니면 **'판매중'**이라고 표시되는 컬럼을 추가하세요.
SELECT *, IF(STOCK = 0, '품절', '판매중') 판매현황 FROM BOOK_STORE;
