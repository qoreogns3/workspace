...
#MAX(), MIN()에 문자가 들어가는 경우 오름차순, 내림차순
#단일행함수 및 일반 컬럼조회는 집계함수와 함께 사용 시 주의 필요 !!!

#ex) 테이블에 데이터가 10개 존재 -> 함수 실행 결과 조회 데이터도 10개
SELECT COMM, IFNULL(COMM, 10) FROM EMP; #단일행함수의 예시
...
# 커미션의 평균을 조회
# AVG() 함수는 평균 구할 때 NULL 데이터는 개수에서 생략 후 계산 !!
SELECT AVG(COMM), SUM(COMM), COUNT(COMM) FROM EMP;

# 커미션의 평균을 조회하되, 모든 사원 수를 기준으로 평균을 조회하라. 소수점 2째자리까지.
# 방법 : 인센이 NULL이면 -> O으로 변경 후 계산
SELECT ROUND(AVG(IFNULL(COMM, 0)), 2) FROM EMP;

#단일행함수 및 일반 컬럼조회는 집계함수와 함께 사용 시 주의 필요 !!!
# 주의 : 조회하는 모든 컬럼은 반드시 조회되는 데이터의 행 갯수가 동일해야 조회가능함!
# 	  -> 결론 : 집게함수는 단일행 함수 및 일반컬럼이랑 함께 조회할 수 없음 !!
...
#예시2. 부서별 사원수를 조회, 사원수가 4명 미만인 부서는 조회 하지 않음
SELECT DEPTNO, COUNT(EMPNO)
FROM EMP
GROUP BY DEPTNO
HAVING COUNT(EMPNO) >= 4;

#Q1. 사번이 짝수인 사원들에 대해 부서별로 급여의 합 및 평균을 조회하되,
#    단, 부서별 급여의 합이 300 이상인 데이터만 조회하고,
#    부서번호 기준 내림차순으로 정렬

#SELECT문은 항상 이 순서대로 작성
...
# SELECT 쿼리문의 해석 순서 (컴퓨터가 해석 순서) *** 매우 중요 !!!!
